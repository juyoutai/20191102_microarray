...
# by colour
ggplot(as.data.frame(PCA.df_2), aes(PC1, PC2, colour = Pheno)) +
  xlab(paste0("PC1, VarExp: ", percentVar[1], "%")) +
  ylab(paste0("PC2, VarExp: ", percentVar[2], "%")) + 
  geom_point() + theme_bw() +
  geom_text_repel(label = colnames(cDC1_gene_expression))
...
annot <- AnnotationDbi::select(mogene10sttranscriptcluster.db, 
                               ...
                               keytype = "PROBEID")
# you can see many probes are not assigned to any genes
...
heatmap.2(t(as.matrix(heatmap_DEG)), col = Seurat, trace = "none", 
          cexCol = 0.9, cexRow = 0.5, scale = "col", RowSideColors = c())
